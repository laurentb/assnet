...
class ConfigCLITest(TestCase):
    ...
    def test_resolve(self):
        for p in (os.path.join(self.root, 'penguins'), os.path.join(self.root, 'penguins' + os.path.sep)):
            self.beginCapture()
            assert self.app.main(['assnet_test', 'config', '-f', p, 'resolve']) in (0, None)
        ...
        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-f', os.path.join(self.root, 'penguins', 'gentoo'), 'resolve']) in (0, None)
        output = self.endCapture()
        assert output.startswith('/penguins/gentoo => ')
        assert output.strip().endswith('/090ec30f855081df160d60cbfe635c399b5ff523')

        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-f', self.root, 'resolve']) in (0, None)
        output = self.endCapture()
        assert output.startswith(' => ')
        assert output.strip().endswith('/da39a3ee5e6b4b0d3255bfef95601890afd80709')

        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-g', 'resolve']) in (0, None)
        output = self.endCapture()
        assert output.startswith('<global> => ')
        assert output.strip().endswith('/config')

        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-u', 'penguin', 'resolve']) in (0, None)
        output = self.endCapture()
    ...
    def test_configList(self):
        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-g', 'list']) in (0, None)
        output = self.endCapture()
        assert 'storage.version' in output

        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-f', self.root, 'list']) in (0, None)
        output = self.endCapture()
        assert len(output)  # it has the default perms

        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-u', 'penguin', 'list']) in (0, None)
    ...
    def test_filterList(self):
        ...
        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-u', 'penguin', 'list', 'auth']) in (0, None)
        output = self.endCapture()
        assert len(output)
    ...
    def test_getAndSet(self):
        ...
        assert self.app.main(['assnet_test', 'config', '-g', 'get', 'lol.cat']) == 2
        output = self.endCapture()
        assert output.strip() == 'Error: lol.cat is undefined.'
        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-g', 'set', 'lol.cat', '42']) in (0, None)
        output = self.endCapture()
        assert len(output) == 0

        self.beginCapture()
        assert self.app.main(['assnet_test', 'config', '-g', 'get', 'lol.cat']) in (0, None)
        output = self.endCapture()
